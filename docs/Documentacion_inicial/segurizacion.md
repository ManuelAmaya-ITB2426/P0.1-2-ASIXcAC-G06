...
## Firewall (davant S1):
**Eleccion:** AWS Security Groups + iptables (SG externa, nftables host).
...
| iptables | Nativo Amazon Linux 2/Docker, estable (sin conflictos daemon.json), simple rules DROP ports no web. Persistente service iptables save. | **nftables:** Conflicto Docker (rompe NAT/bridge, exit-code 1). UFW: No AL2/Docker. firewalld: Overhead extra. |
