...
- **Creación del Contenedor:** Comando base para ejecutar el contenedor S1. Se utiliza el volumen en modo read-only (:ro) para asegurar que el contenedor no modifique accidentalmente la configuración maestra
  ```sql
  docker run -d \
    --name s1-s1-nginx-1 \
