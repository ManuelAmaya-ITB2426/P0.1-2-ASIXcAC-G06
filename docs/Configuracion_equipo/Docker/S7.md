...
# Sprint 4 - Securitzar i habilitar logs

## 5. Habilitar logs.

Para poder controlar mejor lo que pasa en la bbdd hemos habilitado un log que se configura con cada reinicio o recreacion de la bbdd.

Los logs se han configurado en el path **sprint2/S7** donde tabien estan el dockerfile i el .ini de la bbdd. En un archivo llamado my.cnf que contiene las siguientes lineas para hacer que el log se guarde en la maquina fisica i no en el contenedor.
...
I en el docker file hemos añadido como volumen esto para que lo cree de forma automatica con cada recreacion i que los datos se puedan unificar como con los de las tablas.

Una vez se ha recreado el contenedor con la nueva configuración para comprovar que la configuracion se h aplicado hay que ir a la bbdd en el contenedor y ejecutar esto:
- **SHOW VARIABLES LIKE 'general_log';**

I si aparece que esta en **ON** quiere decir que la configuración nueva se ha aprovado.
